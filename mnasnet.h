...
public:
    void forward(float x[in_channels][in_height][in_width], float y[out_channels][out_height][out_width]) {
        ...
        _InvertedResidual<in_channels, in_height, in_width, out_channels, out_height, out_width, kernel_size, stride, expansion_factor> l0_invres;

        float yi[out_channels][out_height][out_width];
        l0_invres.forward(x, yi);

        for (int i = 1; i < repeats; i++) {
            _InvertedResidual<out_channels, out_height, out_width, out_channels, out_height, out_width, kernel_size, 1, expansion_factor> li_invres;
