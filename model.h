...
void FeatureExtractor(const qaint x[3 * test_image_height * test_image_width],
    ...
    // qaint x0[3 * test_image_height * test_image_width]; // maybe unnecessary
    ...
    Conv2d(x, y0, "layer1.0", 3, test_image_height, test_image_width, l0_out_channels, l0_out_height, l0_out_width, l0_kernel_size, l0_stride, l0_padding, l0_groups, apply_bias);
    ...
    ofstream ofs5("./results-qt2/layer-y0-00009.txt");
    ...
    // for (int idx = 0; idx < channels_5 * height_32 * width_32; idx++)
        // ofs5 << layer5[idx] / (float) (1 << actshifts[act_cnt]) << "\n";
        ofs5 << y3[idx] / (float) (1 << actshifts[act_cnt]) << "\n";
