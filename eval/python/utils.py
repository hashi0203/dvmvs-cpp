...
def get_non_differentiable_rectangle_depth_estimation(reference_pose_torch,
    ...
    points_3d_src = kornia2.depth_to_3d(previous_depth_torch, full_K_torch, normalize_points=False)
    points_3d_src = points_3d_src.permute(0, 2, 3, 1)
    points_3d_dst = kornia2.transform_points(trans[:, None], points_3d_src)
    ...
    projections = torch.round(kornia2.project_points(points_3d_dst, half_K_torch.unsqueeze(1))).long()
    ...
    depth_hypothesis = torch.zeros(size=(batch_size, 1, half_height, half_width)).to(device)
    for projection_index in range(0, batch_size):
        ...
        ij_combined_unique_indices = torch.from_numpy(ij_combined_unique_indices).long().to(device)
...
#     points_3d_src = kornia2.depth_to_3d(previous_depth_torch, full_K_torch, normalize_points=False)
#     points_3d_src = points_3d_src.permute(0, 2, 3, 1)
#     points_3d_dst = kornia2.transform_points(trans[:, None], points_3d_src).view(batch_size, -1, 3)
...
def warp_frame_depth(
    ...
    points_3d_dst: torch.Tensor = kornia2.depth_to_3d(depth_dst, camera_matrix, normalize_points)  # Bx3xHxW

    # transform points from source to destination
    points_3d_dst = points_3d_dst.permute(0, 2, 3, 1)  # BxHxWx3

    # apply transformation to the 3d points
    points_3d_src = kornia2.transform_points(src_trans_dst[:, None], points_3d_dst)  # BxHxWx3
    points_3d_src[:, :, :, 2] = torch.relu(points_3d_src[:, :, :, 2])

    # project back to pixels
    camera_matrix_tmp: torch.Tensor = camera_matrix[:, None, None]  # Bx1x1xHxW
    points_2d_src: torch.Tensor = kornia2.project_points(points_3d_src, camera_matrix_tmp)  # BxHxWx2

    # normalize points between [-1 / 1]
    height, width = depth_dst.shape[-2:]
    points_2d_src_norm: torch.Tensor = kornia2.normalize_pixel_coordinates(points_2d_src, height, width)  # BxHxWx2
        ...
        if len(times) > 0:
            ...
            if filename is not None:
                with open(filename, 'a') as f:
                    f.write("Number of Forward Passes:%d\n" % len(times))
                    f.write("--- Mean Inference Time:%f\n" % mean_time)
                    f.write("--- Std Inference Time:%f\n" % std_time)
                    f.write("--- Median Inference Time:%f\n" % median_time)
                    f.write("--- Min Inference Time:%f\n" % min_time)
                    f.write("--- Max Inference Time:%f\n" % max_time)
        else:
